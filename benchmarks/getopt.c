    ...
    if (nonoption_flags_max_len == 0) {
      if (__getopt_nonoption_flags == NULL
        ...
        if (__getopt_nonoption_flags == NULL)
          ...
          memset(
              __mempcpy(__getopt_nonoption_flags, orig_str, len),
              '\0',
      ...
      if (!strncmp(p->name, nextchar, nameend - nextchar)) {
        if ((unsigned int)(nameend - nextchar)
          ...
          indfound = option_index;
            ...
            long_only || pfound->has_arg != p->has_arg
    ...
    if (temp[0] == 'W' && temp[1] == ';') {
      ...
      if (*nextchar != '\0') {
        ...
        if (print_errors) {
          ...
          __asprintf(
              &buf, _("%s: option requires an argument -- %c\n"), argv[0], c);
